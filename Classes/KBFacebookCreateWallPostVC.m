...
@implementation KBFacebookCreateWallPostVC
...
- (void)viewDidLoad {
	...
	if (![[KBAccountManager sharedInstance] usesFacebook]) {
		...
		if (![self getAuthenticatedUser].sendToFacebook ) {
            [self toggleFacebook];
        }
    }
}
...
- (void) submitTweet {
    ...
    if (replyToStatusId && replyToStatusId > 0) {
		if (isGeotagOn) {
			[[[KBTwitterManager twitterManager] twitterEngine] sendUpdate:tweetTextView.text withLatitude:[[KBLocationManager locationManager] latitude] withLongitude:[[KBLocationManager locationManager] longitude] inReplyTo:[replyToStatusId longLongValue]];
		} else {
			[[[KBTwitterManager twitterManager] twitterEngine] sendUpdate:tweetTextView.text inReplyTo:[replyToStatusId longLongValue]];
		}
    } else {
		if (isGeotagOn) {
