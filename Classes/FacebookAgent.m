...
#pragma mark -
#pragma mark facebook agent delegates
- (void) facebookAgent:(FacebookAgent*)agent loginStatus:(BOOL) loggedIn{
	if(!loggedIn){
		//do nothing.  keep showing yourself
		[[NSNotificationCenter defaultCenter] postNotificationName:@"completedFacebookLogout" object:nil];
	}else {
		//now ask for permissions if needed
		BOOL alreadyPermitted = [self hasPermission:FacebookAgentPermissionStatusUpdate];
		if (alreadyPermitted) {
...
- (void) facebookAgent:(FacebookAgent*)agent permissionGranted:(FacebookAgentPermission)permission{
	if (permission==FacebookAgentPermissionStatusUpdate) {
		[[NSNotificationCenter defaultCenter] postNotificationName:@"completedFacebookLogin" object:nil];
	}
}
- (void) facebookAgent:(FacebookAgent*)agent didLoadInfo:(NSDictionary*) info{
	DLog(@"Got facebook user info: %@",info);
}
